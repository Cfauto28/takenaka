...
/**
 * A provider of Modern Yarn's maven-metadata.xml file.
 *
 ...
 */
class ModernYarnMetadataProvider @Deprecated(
    "Jackson will be an implementation detail in the future.",
    ReplaceWith("ModernYarnMetadataProvider(workspace, relaxedCache)")
    ...
    /**
     * Parses Modern Yarn version strings in the metadata file.
     *
    ...
    private fun readMetadata(): JsonNode {
        ...
        val metadataLocation = "https://repo.codemc.io/repository/relativitymc/org/relativitymc/modern-yarn/maven-metadata.xml"
                ...
                if (it.readText() == file.getChecksum(sha1Digest)) {
                    try {
                        return xmlMapper.readTree(file).apply {
                            logger.info { "read cached Modern Yarn mappings metadata" }
                        }
                    } catch (e: JacksonException) {
                        logger.warn(e) { "failed to read cached Modern Yarn mappings metadata, fetching it again" }
                    }
                } else {
