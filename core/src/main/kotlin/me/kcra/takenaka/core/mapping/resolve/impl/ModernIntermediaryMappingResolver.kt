...
class ModernIntermediaryMappingResolver(
    ...
    override val licenseSource: String = "https://raw.githubusercontent.com/RelativityMC/intermediary/master/LICENSE"
    override val targetNamespace: String = "modern-intermediary"
    ...
    override val mappingOutput = lazyOutput<Path?> {
        resolver {
            ...
            val url = URL("https://raw.githubusercontent.com/RelativityMC/intermediary/master/mappings/${version.id}.tiny")
            val length = url.contentLength

            if (length == -1L) {
                logger.info { "did not find Modern Intermediary mappings for ${version.id}" }
                return@resolver null
            }

            if (MAPPINGS in workspace) {
                if (file.fileSize() == length) {
                    logger.info { "matched same length for cached ${version.id} Modern Intermediary mappings" }
            ...
            withContext(Dispatchers.IO + CoroutineName("resolve-coro")) {
                url.httpRequest {
                    if (it.ok) {
                        ...
                        logger.info { "fetched ${version.id} Modern Intermediary mappings" }
                        return@httpRequest file
                    }

                    logger.warn { "failed to fetch ${version.id} ModernIntermediary mappings, received ${it.responseCode}" }
        ...
        resolver {
            licenseWorkspace.withLock(WORKSPACE_LOCK) {
                ...
                if (LICENSE in licenseWorkspace) {
                    logger.info { "found cached Modern Intermediary license file" }
                    return@withLock file
                }

                URL(licenseSource).copyTo(file) // TODO: use IO context

                logger.info { "fetched Modern Intermediary license file" }
                return@withLock file
            }
        }
